...
function add_user(user){
  ...
  let totalTask = user.length;
  let iTasks = user.filter(item => item.completed === false);
  let totalIncomplete = iTasks.length;
  let incompleteTasks = iTasks.map(item => item.title);
  return {'totalTasks': totalTask, 'totalIncomplete': totalIncomplete, 'incompleteTasks': incompleteTasks}
}

let todo_not_finished = todo_link.filter(item => [3,7,8].includes(item.userId));
console.log(todo_not_finished)
summary = {}
for (userid of [3,7,8]){
  summary['user' + userid] = add_user(todo_not_finished.filter(item => item.userId === userid));
}
